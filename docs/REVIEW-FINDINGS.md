...
43 verified findings. Status: **ALL batches done (last: 2026-07-14)** — including the final 4 behavior-changing items (signed off 2026-07-14): selector `useSyncExternalStore` rework, `useSaaSWorkspaces` → real provider, hook return-shape renames, response-envelope unification (`selectFreePlan`/`requestAuth`).
...
## Final batch + post-review fixes — done (2026-07-14, unreleased)

The 4 remaining behavior-changing findings (checked off in their sections below) plus the findings from the 2026-07-14 review of post-07-10 commits:

- [x] 🟠 **MCP `resources/read` scope bypass** — an under-scoped exact resource fell through to a laxer matching template (repro'd with a scopeless token reading an admin-gated URI). Exact resource now owns its URI; scope failure → -32002, templates never consulted. (`mcp-server.ts`)
- [x] 🟡 **No under-scoped-token tests** — added for resources/list, resources/read (exact + template + overlap), resources/templates/list, prompts/list, prompts/get. (`mcp-server.test.ts`)
- [x] 🟡 **`VerifiedJwtPayload` registered claims were typed but not runtime-validated** — `assertRegisteredClaimTypes` after signature verification; mistyped `sub`/`iss`/`jti`/`exp`/`nbf`/`iat`/`aud` → `invalid_token`. `payload.scope as string[]` cast replaced with a type-guard filter (agent-auth + JSDoc example). (`agent-bridge.ts`)
- [x] 🟡 **`resources/read` missing/non-string `uri`** → -32602 instead of -32002. (`mcp-server.ts`)
- [x] 🟡 **`t` in fetch-callback deps** (from the i18n commit) — locale switch aborted+refetched connected-agents and workspaces with loading flicker; `t` now in a ref. (`connected-agents/hooks.ts`, workspace lifecycle)
- [x] 🟡 **Duplicate `getDisplayCurrency`** — subscription-dialog util delegates variant matching to shared `getPricingVariant` (byte-identical UI fallbacks kept).
- [x] ⚪ 'Hard limit' standalone default decapitalization restored · check-i18n.mjs eval hardened · AGENTS.md links → unpkg · CreditPackagesDialog sub-cent per-credit display · `trimTrailingSlash` dedup · `AnyMcpTool` in agent-stack · agent-stack tests stub `discovery.fetch` (was: 1 test timing out on machines where the fake hostname hangs) · SettingsGeneral alt ×2 + CommandSelect defaults + SettingsUsage K/M via i18n.

- [x] 🟡 **`WorkspaceContextValue` name collision resolved (2026-07-14)** — the second definition (`providers/workspace/types.ts`, a workspace-hook-shaped interface) was dead code with zero references, not exported anywhere. Deleted it; the only remaining `WorkspaceContextValue` is the internal `{ state, dispatch }` store-context type (`contexts/WorkspaceContext/types.ts`), correctly used by SDKContext + `useWorkspaceContext`. No collision, no rename needed.
...
- [x] 🟠 **`useSaaSWorkspaces` singleton logic lives per hook instance.** Done 2026-07-14 — lifecycle + actions moved into `workspace/lifecycle.tsx` `WorkspaceProvider`, mounted by SaaSOSProvider; hook is a selector/actions consumer, return shape unchanged. `workspace/hooks.ts:202-220, 228-230, 233-273, 385-417` — init effect, dedup refs, auto-switch fallback are per-instance while the hook mounts in many components: double fetches, and `onWorkspaceChange` / `workspace:changed` can fire multiple times per switch (implementors mint tokens there). Move lifecycle into a real provider (`WorkspaceProvider` at provider.tsx:45-47 is currently an empty passthrough); keep the hook as selector/actions.
- [x] 🟠 **Selector API doesn't bail out of re-renders.** Done 2026-07-14 — `createContextProvider` rebuilt on `useSyncExternalStore` over a stable store (`createStore`, unit-tested); `useAppSelector` subscribes across the three stores with a cached combined snapshot; `useSelectWithEquality` (render-phase ref writes) deleted. `contexts/shared/createContext.tsx:73-102`, `useAppSelector.ts:35-56` — `useContext` on the whole state context means every dispatch re-renders every consumer of `useSaaSAuth`/`usePermissions`/`useUIVisibility`/`useSaaSWorkspaces`; docs claiming selector bailout are false; `useSelectWithEquality` writes refs during render. Rework with `useSyncExternalStore` over a mutable store (or `use-context-selector`).
...
- [x] 🟡 **Hook return shapes drift from `{ data, loading, error, refetch }` convention.** Done 2026-07-14 — `useUserAttributes`/`useUserFeatures` return per-resource `{ …, loading, error, refetch }` (old names kept as `@deprecated` aliases); `useSaaSSettings` returns `{ settings, loading, error, refetch }` backed by new `settingsStatus` OS state; WorkspaceSwitcher waits for settings instead of `?? true` guessing. `useUserAttributes`/`useUserFeatures` → `isLoading`/`refreshX`; `useSaaSSettings` → no loading/error at all (WorkspaceSwitcher flashes wrong UI on `?? true` defaults until settings arrive); `useSaaSWorkspaces` returns an unmemoized ~25-key object every render (`hooks.ts:618-646`).
