import type { SDKMessages } from '../types';

export const de: SDKMessages = {
  settings: {
    titles: {
      profile: 'Konto',
      security: 'Sicherheit',
      devices: 'Geräte & Sitzungen',
      general: 'Arbeitsbereich-Einstellungen',
      users: 'Arbeitsbereich-Mitglieder',
      subscription: 'Plan & Abrechnung',
      usage: 'Nutzung',
      credits: 'Guthaben',
      features: 'Arbeitsbereich-Funktionen',
      notifications: 'Benachrichtigungen',
      permissions: 'Berechtigungen',
      danger: 'Gefahrenzone',
    },
    sidebar: {
      account: 'Konto',
      workspace: 'Arbeitsbereich',
      profile: 'Konto',
      security: 'Sicherheit',
      devices: 'Geräte',
      general: 'Einstellungen',
      users: 'Mitglieder',
      subscription: 'Plan & Abrechnung',
      usage: 'Nutzung',
      credits: 'Guthaben',
      features: 'Funktionen',
      notifications: 'Benachrichtigungen',
      permissions: 'Berechtigungen',
      danger: 'Gefahrenzone',
    },
    common: {
      refresh: 'Aktualisieren',
      refreshing: 'Wird aktualisiert...',
      retry: 'Erneut versuchen',
      retrying: 'Wird erneut versucht...',
      save: 'Speichern',
      saving: 'Wird gespeichert...',
      cancel: 'Abbrechen',
      delete: 'Löschen',
      dismiss: 'Verwerfen',
      loading: 'Wird geladen...',
      success: 'Erfolg!',
      error: 'Fehler',
      refreshAction: '{loading, select, true {Wird aktualisiert...} other {Aktualisieren}}',
      retryAction: '{loading, select, true {Wird erneut versucht...} other {Erneut versuchen}}',
      noPermissionTitle: 'Zugriff eingeschränkt',
      noPermissionDescription:
        'Sie haben keine Berechtigung, auf diesen Bereich zuzugreifen. Kontaktieren Sie einen Workspace-Administrator, wenn Sie Zugriff benötigen.',
      billingManageOnly:
        'Nur Benutzer mit Abrechnungsverwaltungsberechtigungen können diese Aktion durchführen.',
      openSettings: 'Einstellungen öffnen',
      back: 'Zurück',
      close: 'Schließen',
    },
  },
  subscription: {
    managePlan: 'Verwalte deinen Plan und deine Abrechnung',
    changePlan: 'Plan ändern',
    viewPricingPlans: 'Preispläne ansehen',
    viewPlans: 'Pläne ansehen',
    upgradePlan: 'Jetzt upgraden',
    subscribe: 'Abonnieren',
    currentPlan: 'Aktueller Plan',
    noSubscription: 'Du hast kein aktives Abonnement',
    choosePlan: 'Wähle deinen Plan',
    choosePlanDescription: 'Vergleiche Pläne und wähle den, der zu deinen Anforderungen passt',
    forWorkspace: 'für {name}',
    planUpdateAvailable: 'Plan-Update verfügbar',
    updateSuccess: 'Abonnement erfolgreich aktualisiert!',
    updateFailed: 'Aktualisierung fehlgeschlagen',
    cancelTitle: 'Abonnement kündigen',
    cancelConfirm: 'Bist du sicher, dass du dein Abonnement kündigen möchtest?',
    cancelKeep: 'Abonnement behalten',
    cancelButton: 'Ja, Abonnement kündigen',
    cancelTrialTitle: 'Testversion abbrechen',
    cancelTrialConfirm: 'Bist du sicher, dass du deine Testversion abbrechen möchtest?',
    cancelTrialAccess: 'Deine Testversion läuft weiter bis',
    cancelTrialNoCharge: 'Dir wird nichts berechnet — deine Testversion läuft einfach ab',
    cancelTrialResume: 'Du kannst deine Testversion jederzeit vor Ablauf wieder aufnehmen',
    cancelTrialKeep: 'Testversion behalten',
    cancelTrialButton: 'Ja, Testversion abbrechen',
    canceling: 'Wird gekündigt...',
    cancelSuccess: 'Das Abonnement wird zum Ende des Abrechnungszeitraums gekündigt.',
    cancelNotCharged: 'Nach der Kündigung wird dir nichts mehr berechnet',
    cancelResumeAnytime: 'Du kannst dein Abonnement jederzeit vor Ablauf wieder aufnehmen',
    resumeTitle: 'Abonnement fortsetzen',
    resumeTrial: 'Testversion fortsetzen',
    resumeConfirm: 'Bist du sicher, dass du dein Abonnement fortsetzen möchtest?',
    resumeKeep: 'Gekündigt lassen',
    resumeButton: 'Ja, Abonnement fortsetzen',
    resuming: 'Wird fortgesetzt...',
    resumeSuccess: 'Das Abonnement wurde fortgesetzt.',
    managePayment: 'Zahlung verwalten',
    openingPortal: 'Wird geöffnet...',
    viewAllPlans: 'Alle Pläne ansehen',
    scheduledCancellation: 'Kündigung des Abonnements geplant',
    scheduledTrialCancellation: 'Kündigung der Testversion geplant',
    paymentPastDue: 'Zahlung überfällig',
    paymentPastDueDescription:
      'Bitte aktualisiere deine Zahlungsmethode, um eine Dienstunterbrechung zu vermeiden.',
    subscriptionPaused: 'Abonnement pausiert',
    subscriptionPausedDescription:
      'Dein Abonnement ist derzeit pausiert. Setze es fort, um den vollen Zugang wiederherzustellen.',
    errorLoading: 'Fehler beim Laden der Abonnementdaten',
    errorLoadingDescription: 'Bitte überprüfe deine Verbindung und versuche es erneut.',
    errorProcessing: 'Abonnement konnte nicht verarbeitet werden',
    errorCanceling: 'Abonnement konnte nicht gekündigt werden',
    errorResuming: 'Abonnement konnte nicht fortgesetzt werden',
    invalidWorkspace: 'Ungültige Arbeitsbereich-ID',
    billingInterval: {
      monthly: 'Monatlich',
      quarterly: 'Vierteljährlich',
      yearly: 'Jährlich',
      perMonth: '/Monat',
      perQuarter: '/Quartal',
      perYear: '/Jahr',
    },
    seats: {
      title: 'Plätze',
      members: 'Mitglieder',
      included: 'Enthalten',
      billable: 'Abrechenbar',
      perExtraSeat: 'Pro zusätzlichem Platz',
      limit: 'Limit',
      includedSeats: 'Enthaltene Plätze',
      maxSeats: 'Max. Plätze',
      freeWithBase: 'Im Grundpreis enthalten',
      allIncluded: 'Alle enthalten',
      extra: 'zusätzlich',
      unlimited: 'Unbegrenzt',
    },
    items: {
      features: 'Funktionen',
      limits: 'Limits',
      quotas: 'Nutzungskontingente',
      credits: 'KI-Guthaben',
      creditsPerMonth: 'Guthaben pro Monat',
      creditsOneTime: 'Guthaben (einmalig)',
      creditRenewal: 'Guthaben-Erneuerung',
      creditModeLifetime: 'Einmalig gewährt',
      creditsPerPeriod: 'Guthaben pro Monat',
      creditRollover: 'Guthaben-Erneuerung',
      creditModeReset: 'Jeden Monat neu',
      creditModeTopup: 'Wird jeden Monat aufgestockt',
    },
    checkout: {
      upgrade: 'Upgraden',
      downgrade: 'Downgraden',
      switchTo: 'Wechseln zu',
      select: 'Auswählen',
    },
    status: {
      active: 'Aktiv',
      canceling: 'Wird gekündigt',
      trial: 'Testversion',
      canceled: 'Gekündigt',
      pastDue: 'Überfällig',
      paused: 'Pausiert',
      deprecated: 'Veraltet',
      unknown: 'Unbekannt',
    },
    plan: 'Plan',
    invoicesTab: 'Rechnungen',
    tabsLabel: 'Abonnement-Tabs',
    endsOn: '(endet {date})',
    versionLabel: 'Version {version}',
    changePlanButton: 'Plan ändern',
    noPlanAssigned: 'Kein Plan zugewiesen',
    free: 'Kostenlos',
    perSeatDisplay: '+ {price}/Platz{interval}',
    estTotalDisplay:
      'Geschätztes Gesamt: {total}{interval} ({count, plural, one {# zusätzlicher Platz} other {# zusätzliche Plätze}})',
    nextBillingDisplay: 'Nächste Abrechnung: {date}',
    membersInWorkspace: '{count, plural, one {# Mitglied} other {# Mitglieder}} im Arbeitsbereich',
    billingInCurrency: 'Abrechnung in {currency}',
    savingsPercent: '{percent, number}% sparen',
    switchToInterval: 'Zu {interval} wechseln',
    seatPriceDisplay: '+ {price}/Platz{interval} ({included} {includedLabel})',
    trialEnded: 'Deine Testversion ist abgelaufen.',
    trialEndsIn: 'Deine Testversion endet in {days, plural, one {# Tag} other {# Tagen}}.',
    onTrial: 'Du befindest dich in der Testversion.',
    upgradeToKeepAccess: 'Upgrade, um den Zugang zu behalten.',
    viewInvoices: 'Rechnungen ansehen',
    cancelTrial: 'Testversion abbrechen',
    dunningFinal: 'Letzte Warnung: Abonnement wird ausgesetzt',
    dunningSuspended: 'Abonnement ausgesetzt',
    dunningRecovery: 'Zahlungswiederherstellung läuft',
    dunningSuspendedDescription:
      'Dein Abonnement wurde aufgrund wiederholter Zahlungsfehler ausgesetzt. Aktualisiere deine Zahlungsmethode, um den Zugang wiederherzustellen.',
    dunningRecoveryDescription:
      'Wir konnten deine Zahlung nicht verarbeiten. Bitte aktualisiere deine Zahlungsmethode, um den Zugang nicht zu verlieren.',
    cancelEndDescription: 'Dein Abonnement endet am',
    trialEndDescription: 'Deine Testversion endet am',
    cancelEndFallback:
      'Dein Abonnement wird zum Ende des aktuellen Abrechnungszeitraums gekündigt. Dir wird nichts mehr berechnet.',
    resumeChargeDate: 'Dir wird erneut berechnet am',
    resumeChargeFallback: 'dem nächsten Abrechnungsdatum',
    resumeContinue:
      'Dein Abonnement wird automatisch fortgesetzt und gemäß deinem Plan abgerechnet',
    resumeCancelAnytime:
      'Du kannst jederzeit vor dem nächsten Abrechnungsdatum kündigen, wenn du es dir anders überlegst',
    retainAccess: 'Du behältst den vollen Zugang zu diesem Plan bis',
    retainAccessFallback: 'zum Ende deines aktuellen Abrechnungszeitraums',
    noPlansAvailable:
      'Für diesen Arbeitsbereich sind keine Plangruppen verfügbar. Bitte kontaktiere den Support, wenn du glaubst, dass dies ein Fehler ist.',
    noPlansAvailableHint: 'Für diesen Arbeitsbereich sind keine Plangruppen verfügbar.',
    startTrialDescription:
      'Teste den vollständigen Plan ohne Verpflichtung. Du kannst jederzeit upgraden oder kündigen.',
    noSubscriptionDescription: 'Wähle einen Plan, um mit deinem Arbeitsbereich loszulegen',
    startFreeTrial: 'Kostenlose Testversion starten',
    startTrialDays: '{days}-tägige kostenlose Testversion starten',
    trialBadge: '{days}-Tage-Testversion',
  },
  users: {
    emailPlaceholder: 'example@example.com',
    title: 'Arbeitsbereich-Mitglieder',
    adminOnly: 'Nur der Arbeitsbereich-Administrator kann Benutzer und Rollen verwalten.',
    inviteByEmail: 'Mitglied per E-Mail einladen',
    inviteButton: 'Einladen',
    inviting: 'Wird eingeladen...',
    inviteSuccess: 'Benutzer erfolgreich eingeladen',
    emailRequired: 'E-Mail-Adresse ist erforderlich',
    invalidEmail: 'Ungültiges E-Mail-Adressformat',
    inviteFailed: 'Einladung des Mitglieds fehlgeschlagen',
    seatLimitReached: 'Platzlimit erreicht',
    memberLimitReached: 'Mitgliederlimit erreicht',
    role: 'Rolle',
    you: 'Du',
    owner: 'Eigentümer',
    noMembers: 'Keine Mitglieder in diesem Arbeitsbereich gefunden.',
    extraSeatCost:
      'Jedes zusätzliche Mitglied fügt {price}/{interval, select, yearly {Jahr} quarterly {Quartal} other {Monat}} zu deinem Abonnement hinzu',
    unknownUser: 'Unbekannter Benutzer',
    seats: 'Plätze',
    members: 'Mitglieder',
    included: 'enthalten',
    perExtraSeat: '/zusätzlicher Platz',
    maxMembers: 'max. Mitglieder',
    available: 'verfügbar',
    noMemberLimit: 'Kein Mitgliederlimit',
    seatHeading: '{hasSeatPricing, select, true {Plätze} other {Mitglieder}}',
    limitReached:
      '{reason, select, seat_limit_reached {Platzlimit erreicht} other {Mitgliederlimit erreicht}}',
    memberCount: '{count, plural, one {# Mitglied} other {# Mitglieder}}',
    subscriptionRequired: 'Ein Abonnement ist erforderlich, um Mitglieder einzuladen.',
    seatLimitMessage:
      'Dein Plan erlaubt bis zu {maxUsers, number} Mitglieder. Upgrade für mehr Plätze.',
    settingsLimitMessage: 'Dieser Arbeitsbereich erlaubt bis zu {maxUsers, number} Mitglieder.',
    memberLimitMessage: 'Mitgliederlimit von {maxUsers, number} erreicht.',
    selectRole: 'Rolle auswählen',
    inviteAs: 'Einladen als {role}',
  },
  invoices: {
    title: 'Abrechnungsverlauf',
    description: 'Rechnungen ansehen und herunterladen',
    pay: 'Bezahlen',
    view: 'Ansehen',
    viewDetails: 'Details ansehen',
    download: 'PDF',
    errorLoading: 'Fehler beim Laden der Rechnungen',
    noInvoices: 'Noch keine Rechnungen. Abonniere einen Plan, um Rechnungen zu erhalten.',
    noInvoicesWithSub: 'Keine Rechnungen mit Download-Option gefunden',
    paidAmount: 'Bezahlt: {amount}',
    na: 'k. A.',
    status: {
      draft: 'Entwurf',
      open: 'Offen',
      paid: 'Bezahlt',
      uncollectible: 'Uneinbringlich',
      void: 'Storniert',
    },
  },
  usage: {
    title: 'Nutzung',
    description: 'Überwache deinen Ressourcenverbrauch und deine Kontingente',
    totalResources: 'Ressourcen gesamt',
    overallUsage: 'Gesamtnutzung',
    inOverage: 'Im Überschuss',
    estOverageCost: 'Geschätzte Überschusskosten',
    overageDetected: 'Überschuss erkannt',
    overageDescription:
      'Zusätzliche Nutzung wird als Überschuss auf deiner nächsten Rechnung berechnet.',
    overageUnits: 'Überschusseinheiten',
    rate: 'Tarif',
    billableBlocks: 'Abrechenbare Blöcke',
    estimatedCharge: 'Geschätzte Kosten',
    noData: 'Keine Kontingentnutzungsdaten für diesen Arbeitsbereich verfügbar.',
    noDataHint: 'Kontingente erscheinen hier, sobald dein Plan gemessene Ressourcen enthält.',
    used: 'genutzt',
    remaining: 'verbleibend',
    overLimit: 'über dem Limit',
    percentUsed: '{percent, number}% genutzt',
    availableRemaining: '{count} verbleibend',
    overLimitCount: '{count} über dem Limit',
    overageWarning:
      '{count, plural, one {# Ressource hat das enthaltene Kontingent überschritten.} other {# Ressourcen haben das enthaltene Kontingent überschritten.}} Zusätzliche Nutzung wird als Überschuss auf deiner nächsten Rechnung berechnet.',
    fullyUsed: 'Vollständig genutzt',
    renewingSoon: 'wird bald erneuert',
    almostFull: 'Fast voll',
    overage: 'Überschuss',
    errorLoading: 'Nutzungsdaten konnten nicht geladen werden',
    perUnit: '/ Einheit',
    perUnits: '/ Einheiten',
    rateDisplay: '{rate} {unitSize, plural, =1 {/ Einheit} other {/ {unitSize, number} Einheiten}}',
    resourcesExceeded:
      '{count, plural, one {# Ressource hat das enthaltene Kontingent überschritten.} other {# Ressourcen haben das enthaltene Kontingent überschritten.}}',
    estOverageCharges: 'Geschätzte Überschusskosten in diesem Zeitraum: {amount}',
    overageRateDisplay: 'Überschusstarif: {rate}',
    resetDateDisplay: 'Nutzung wird zurückgesetzt am {date}',
    daysRemainingDisplay: '{count, plural, one {# Tag verbleibend} other {# Tage verbleibend}}',
  },
  profile: {
    email: 'E-Mail',
    name: 'Name',
    language: 'Sprache',
    country: 'Land',
    currency: 'Währung',
    timezone: 'Zeitzone',
    profileImage: 'Profilbild',
    success: 'Profil erfolgreich gespeichert',
    nameMinLength: 'Der Name muss mindestens 2 Zeichen lang sein.',
    save: 'Speichern',
    reset: 'Zurücksetzen',
  },
  general: {
    ownerOnly: 'Nur der Eigentümer des Arbeitsbereichs kann die Einstellungen ändern.',
    name: 'Name',
    namePlaceholder: 'Mein Arbeitsbereich',
    nameMinLength: 'Der Arbeitsbereichname muss mindestens 2 Zeichen lang sein.',
    billingCurrency: 'Abrechnungswährung',
    icon: 'Symbol',
    iconDescription:
      'Wähle ein Emoji oder lade ein benutzerdefiniertes Bild für deinen Arbeitsbereich hoch.',
    imageUrl: 'Bild-URL',
    imageUrlPlaceholder: 'https://beispiel.de/bild.png',
    imageUrlDescription:
      'Gib eine gültige URL für dein Arbeitsbereich-Bild ein. Unterstützt PNG-, JPG- und SVG-Formate.',
    previewLabel: 'Vorschau:',
    previewImageAlt: 'Arbeitsbereich-Vorschau',
    success: 'Arbeitsbereich-Einstellungen erfolgreich gespeichert',
    chooseEmoji: 'Emoji wählen',
    customImageUrl: 'Benutzerdefinierte Bild-URL',
    updateWorkspace: 'Arbeitsbereich aktualisieren',
  },
  features: {
    title: 'Arbeitsbereich-Funktionen',
    ownerOnly: 'Nur der Eigentümer des Arbeitsbereichs kann die Funktionen ändern.',
    noFeatures: 'Der Arbeitsbereich hat keine zu verwaltenden Funktionen.',
    enabled: 'aktiviert',
    disabled: 'deaktiviert',
    enabling: 'Wird aktiviert',
    disabling: 'Wird deaktiviert',
    featureSuccess: 'erfolgreich',
    updateSuccess: '{feature} erfolgreich {enabled, select, true {aktiviert} other {deaktiviert}}',
    actionStatus:
      '{action, select, enabling {Wird aktiviert} disabling {Wird deaktiviert} other {}}',
    fallbackName: 'Funktion',
  },
  danger: {
    title: 'Arbeitsbereich löschen',
    adminOnly: 'Nur Administratoren können Arbeitsbereiche löschen.',
    deleteWorkspace: 'Arbeitsbereich löschen',
    deleteConfirm: 'Bist du dir absolut sicher?',
    deleteWarning:
      'Diese Aktion kann nicht rückgängig gemacht werden. Der Arbeitsbereich und alle zugehörigen Daten werden dauerhaft gelöscht.',
    deleting: 'Wird gelöscht...',
    deleteDescription:
      'Sobald du einen Arbeitsbereich löschst, gibt es kein Zurück. Der Arbeitsbereich und alle seine Daten werden dauerhaft gelöscht, einschließlich Mitglieder, Einstellungen und Funktionen.',
    deleteConfirmDescription:
      'Diese Aktion kann nicht rückgängig gemacht werden. Der Arbeitsbereich und alle seine Daten werden dauerhaft gelöscht.',
    failedToDelete: 'Arbeitsbereich konnte nicht gelöscht werden',
  },
  notifications: {
    title: 'Benachrichtigungseinstellungen',
    permissionDenied: 'Benachrichtigungsberechtigung verweigert',
    notSupported: 'Push-Benachrichtigungen werden in diesem Browser nicht unterstützt.',
    manageDescription: 'Verwalte, wie du Benachrichtigungen von diesem Arbeitsbereich erhältst.',
    blocked: 'Benachrichtigungen blockiert',
    blockedDescription:
      'Du hast Benachrichtigungen für diese Seite zuvor blockiert. Um sie zu aktivieren, aktualisiere deine Browsereinstellungen:',
    pushTitle: 'Push-Benachrichtigungen',
    pushEnabledDescription: 'Du erhältst Browser-Benachrichtigungen für wichtige Updates.',
    pushDisabledDescription:
      'Erhalte Benachrichtigungen über Zahlungsprobleme, Ablauf der Testversion und andere wichtige Updates.',
    enabling: 'Wird aktiviert...',
    disabling: 'Wird deaktiviert...',
    enable: 'Aktivieren',
    disable: 'Deaktivieren',
    toggleAction:
      '{loading, select, true {{subscribed, select, true {Wird deaktiviert...} other {Wird aktiviert...}}} other {{subscribed, select, true {Deaktivieren} other {Aktivieren}}}}',
    pushDescription:
      '{subscribed, select, true {Du erhältst Browser-Benachrichtigungen für wichtige Updates.} other {Erhalte Benachrichtigungen über Zahlungsprobleme, Ablauf der Testversion und andere wichtige Updates.}}',
    deviceNote:
      'Benachrichtigungen werden an dieses Gerät gesendet. Aktiviere sie auf anderen Geräten separat.',
    prefsSaved: 'Benachrichtigungseinstellungen gespeichert.',
    loadingPrefs: 'Einstellungen werden geladen...',
    categories: {
      workspace: 'Arbeitsbereich',
      billing: 'Abrechnung & Zahlungen',
    },
    events: {
      workspace_invite: 'Mitglied eingeladen',
      workspace_invite_desc: 'Wenn ein neues Mitglied zum Arbeitsbereich hinzugefügt wird',
      workspace_removed: 'Mitglied entfernt',
      workspace_removed_desc: 'Wenn ein Mitglied aus dem Arbeitsbereich entfernt wird',
      workspace_role_changed: 'Rolle geändert',
      workspace_role_changed_desc: 'Wenn die Rolle eines Mitglieds aktualisiert wird',
      subscription_created: 'Abonnement erstellt',
      subscription_created_desc: 'Wenn ein neues Abonnement aktiviert wird',
      subscription_upgraded: 'Plan hochgestuft',
      subscription_upgraded_desc: 'Wenn der Abonnement-Plan hochgestuft wird',
      subscription_canceled: 'Abonnement gekündigt',
      subscription_canceled_desc: 'Wenn das Abonnement gekündigt wird',
      subscription_suspended: 'Zugriff ausgesetzt',
      subscription_suspended_desc:
        'Wenn der Zugriff wegen fehlgeschlagener Zahlungen ausgesetzt wird',
      trial_ending: 'Testversion endet bald',
      trial_ending_desc: 'Einige Tage bevor die Testversion abläuft',
      trial_expired: 'Testversion abgelaufen',
      trial_expired_desc: 'Wenn die Testphase beendet ist',
      payment_failed: 'Zahlung fehlgeschlagen',
      payment_failed_desc: 'Wenn eine Zahlung nicht verarbeitet werden konnte',
      payment_action_required: 'Zahlungsaktion erforderlich',
      payment_action_required_desc: 'Wenn eine Zahlung eine zusätzliche Bestätigung erfordert',
      payment_succeeded: 'Zahlung erhalten',
      payment_succeeded_desc: 'Wenn eine Zahlung erfolgreich verarbeitet wird',
    },
    unblock: {
      firefox: {
        step1: 'Klicke auf das Schloss-Symbol (oder Schild-Symbol) in der Adressleiste',
        step2: 'Finde "Benachrichtigungen" in der Berechtigungsliste',
        step3: 'Ändere von "Blockiert" zu "Erlauben"',
        step4: 'Lade diese Seite neu',
      },
      safari: {
        step1: 'Öffne Safari → Einstellungen → Websites → Benachrichtigungen',
        step2: 'Finde diese Website in der Liste',
        step3: 'Ändere von "Ablehnen" zu "Erlauben"',
        step4: 'Lade diese Seite neu',
      },
      edge: {
        step1: 'Klicke auf das Schloss-Symbol in der Adressleiste',
        step2: 'Klicke auf "Berechtigungen für diese Website"',
        step3: 'Setze "Benachrichtigungen" auf "Erlauben"',
        step4: 'Lade diese Seite neu',
      },
      chrome: {
        step1: 'Klicke auf das Schloss-Symbol (oder Einstellungssymbol) in der Adressleiste',
        step2: 'Klicke auf "Website-Einstellungen"',
        step3: 'Setze "Benachrichtigungen" auf "Erlauben"',
        step4: 'Lade diese Seite neu',
      },
    },
  },
  quota: {
    includedOnly: '{count, number} enthalten',
    includedWithOverage: '{count, number} enthalten, dann {price} / {unit}',
    includedHardLimit: '{count, number} enthalten (festes Limit)',
    unitFallback: 'Einheit',
  },
  pricing: {
    title: 'Wähle deinen Plan',
    description: 'Vergleiche Pläne und wähle den, der zu deinen Anforderungen passt',
    currency: 'Währung',
    billingIn: 'Abrechnung in',
    noPlansCurrency: 'Keine Pläne für deine Abrechnungswährung verfügbar.',
    noPlansCurrencyHint: 'Etwas ist schiefgelaufen. Bitte kontaktiere den Support.',
    noPlans: 'Keine Pläne verfügbar',
    free: 'Kostenlos',
    included: 'enthalten',
    save: 'Sparen',
    current: 'Aktuell',
    processing: 'Wird verarbeitet...',
    unavailable: 'Nicht verfügbar',
    selectCurrency: 'Abrechnungswährung auswählen',
    billingInterval: 'Abrechnungsintervall',
  },
  beta: {
    logoAlt: 'Logo',
    nameLabel: 'Name',
    emailLabel: 'E-Mail',
    submitText: 'Absenden',
    submittingText: 'Wird gesendet...',
    errorMessage: 'Ein Fehler ist aufgetreten',
    tryAgain: 'Erneut versuchen',
    privacyConsent: 'Mit dem Absenden dieses Formulars stimmst du unserer',
    privacyPolicy: 'Datenschutzrichtlinie',
    and: 'und',
    termsOfService: 'Nutzungsbedingungen',
  },
  workspace: {
    switchTitle: 'Arbeitsbereich wechseln',
    switchDescription: 'Wähle einen Arbeitsbereich aus oder erstelle einen neuen.',
    notLoggedIn:
      'Es sieht so aus, als wärst du nicht eingeloggt. Bitte melde dich an, um fortzufahren.',
    availableWorkspaces: 'Verfügbare Arbeitsbereiche ({count, number})',
    loadingWorkspaces: 'Arbeitsbereiche werden geladen...',
    searchPlaceholder: 'Arbeitsbereiche suchen...',
    noWorkspacesFound: 'Keine Arbeitsbereiche gefunden',
    noWorkspacesAvailable: 'Keine Arbeitsbereiche verfügbar',
    switchTo: 'Wechseln zu',
    membersCount: '{count, plural, one {# Mitglied} other {# Mitglieder}}',
    createNew: 'Neuen Arbeitsbereich erstellen',
    createTitle: 'Neuen Arbeitsbereich erstellen',
    createDescription: 'Erstelle einen neuen Arbeitsbereich, um loszulegen.',
    workspaceName: 'Name des Arbeitsbereichs',
    workspaceNamePlaceholder: 'Mein Arbeitsbereich',
    workspaceNameMinLength: 'Der Arbeitsbereichname muss mindestens 2 Zeichen lang sein.',
    workspaceIcon: 'Arbeitsbereich-Symbol',
    iconDescription:
      'Wähle ein Emoji oder lade ein benutzerdefiniertes Bild für deinen Arbeitsbereich hoch.',
    chooseEmoji: 'Emoji wählen',
    customImageUrl: 'Benutzerdefinierte Bild-URL',
    imageUrl: 'Bild-URL',
    imageUrlPlaceholder: 'https://beispiel.de/bild.png',
    imageUrlDescription:
      'Gib eine gültige URL für dein Arbeitsbereich-Bild ein. Unterstützt PNG-, JPG- und SVG-Formate.',
    invalidUrl: 'Bitte gib eine gültige URL ein',
    creating: 'Wird erstellt...',
    createWorkspace: 'Arbeitsbereich erstellen',
  },
  dropdowns: {
    search: 'Suchen...',
    chooseOption: 'Option wählen',
    chooseCountry: 'Land wählen',
    searchCountry: 'Land wählen...',
    chooseLanguage: 'Sprache wählen',
    searchLanguage: 'Sprache wählen...',
    chooseCurrency: 'Währung wählen',
    searchCurrency: 'Währung wählen...',
    chooseTimezone: 'Zeitzone wählen',
    searchTimezone: 'Zeitzone wählen...',
    noResults: 'Keine Ergebnisse gefunden',
  },
  push: {
    failedToSubscribe: 'Push-Benachrichtigungen konnten nicht abonniert werden',
    failedToUnsubscribe: 'Push-Benachrichtigungen konnten nicht abbestellt werden',
  },
  permissions: {
    title: 'Berechtigungen',
    description: 'Konfiguriere, was jede Rolle in diesem Arbeitsbereich tun kann.',
    ownerOnly: 'Nur der Eigentümer des Arbeitsbereichs kann Berechtigungen verwalten.',
    save: 'Berechtigungen speichern',
    saving: 'Wird gespeichert...',
    saveSuccess: 'Berechtigungen erfolgreich gespeichert',
  },
  credits: {
    title: 'Guthaben',
    description: 'Guthaben kann für Premium-Aktionen wie KI-Generierung verwendet werden.',
    balance: 'Guthabenstand',
    available: 'Verfügbar',
    totalGranted: 'Gesamt gewährt',
    totalConsumed: 'Gesamt verbraucht',
    totalExpired: 'Gesamt abgelaufen',
    noCredits: 'Noch kein Guthaben',
    noCreditsHint:
      'Guthaben wird hier angezeigt, wenn es durch deinen Plan gewährt oder gekauft wurde.',
    buyCredits: 'Guthaben kaufen',
    buyAmount: '{count} Guthaben kaufen',
    purchasing: 'Weiterleitung...',
    packages: 'Guthabenpakete',
    noPackages: 'Keine Guthabenpakete verfügbar.',
    creditsAmount: '{count} Guthaben',
    validityDays: 'Gültig für {count} Tage',
    validityUnlimited: 'Kein Ablaufdatum',
    transactions: 'Letzte Transaktionen',
    noTransactions: 'Noch keine Transaktionen.',
    loadMore: 'Mehr laden',
    viewAll: 'Alle anzeigen',
    expiringSoon: 'Läuft bald ab',
    expiringInDays: '{count} Guthaben läuft innerhalb von {days} Tagen ab',
    noExpiring: 'Kein Guthaben läuft bald ab.',
    type: {
      plan_grant: 'Plan-Gewährung',
      pack_purchased: 'Gekauft',
      consumed: 'Verbraucht',
      expired: 'Abgelaufen',
      admin_grant: 'Gewährt',
      admin_revoke: 'Widerrufen',
      refund: 'Erstattung',
    },
    dialogTitle: 'Guthabenpakete kaufen',
    dialogDescription: 'Wähle ein Guthabenpaket zum Kauf aus.',
    dialogDescriptionWorkspace: 'Guthaben für {workspace} kaufen.',
    perCredit: '{price} pro Guthaben',
    purchaseSuccess: 'Guthaben erfolgreich gekauft! Dein Kontostand wurde aktualisiert.',
    purchaseCanceled: 'Guthabenkauf wurde abgebrochen. Es wurden keine Gebühren erhoben.',
    purchaseFailed:
      'Guthabenkauf fehlgeschlagen. Bitte versuche es erneut oder kontaktiere den Support.',
    errorLoading: 'Guthabendaten konnten nicht geladen werden',
    insufficientCredits: 'Nicht genügend Guthaben',
    insufficientDetail: 'Nicht genügend Guthaben. Verfügbar: {available}, angefordert: {requested}',
  },
  loading: {
    restoringSession: 'Sitzung wird wiederhergestellt...',
    redirecting: 'Weiterleitung...',
    signingIn: 'Anmeldung läuft...',
    verifyingAccount: 'Konto wird überprüft...',
    almostThere: 'Fast geschafft...',
  },
  errors: {
    networkError: 'Netzwerkfehler: Bitte überprüfe deine Internetverbindung',
    unauthorized: 'Nicht autorisiert — Bitte überprüfe deine Sitzung',
    notFound: 'Ressource nicht gefunden',
    generic: 'Ein Fehler ist aufgetreten',
    fetchPlans: 'Pläne konnten nicht abgerufen werden',
    fetchPlanGroup: 'Plangruppe konnte nicht abgerufen werden',
    fetchPlanGroupVersion: 'Plangruppenversion konnte nicht abgerufen werden',
    fetchPlanGroupVersions: 'Plangruppenversionen konnten nicht abgerufen werden',
    fetchSubscription: 'Abonnement konnte nicht abgerufen werden',
    createCheckout: 'Checkout-Sitzung konnte nicht erstellt werden',
    updateSubscription: 'Abonnement konnte nicht aktualisiert werden',
    fetchInvoices: 'Rechnungen konnten nicht abgerufen werden',
    fetchInvoice: 'Rechnung konnte nicht abgerufen werden',
    openBillingPortal: 'Abrechnungsportal konnte nicht geöffnet werden',
    cancelSubscription: 'Abonnement konnte nicht gekündigt werden',
    resumeSubscription: 'Abonnement konnte nicht fortgesetzt werden',
    recordUsage: 'Nutzung konnte nicht erfasst werden',
    fetchQuotaUsage: 'Kontingentnutzungsstatus konnte nicht abgerufen werden',
    fetchAllQuotaUsage: 'Gesamte Kontingentnutzung konnte nicht abgerufen werden',
    fetchUsageLogs: 'Nutzungsprotokolle konnten nicht abgerufen werden',
    fetchCreditBalance: 'Guthabenstand konnte nicht abgerufen werden',
    consumeCredits: 'Guthaben konnte nicht verbraucht werden',
    purchaseCredits: 'Guthaben konnte nicht gekauft werden',
    fetchCreditPackages: 'Guthabenpakete konnten nicht abgerufen werden',
    fetchCreditTransactions: 'Guthabentransaktionen konnten nicht abgerufen werden',
    fetchExpiringCredits: 'Ablaufendes Guthaben konnte nicht abgerufen werden',
    fetchWorkspaces: 'Arbeitsbereiche konnten nicht geladen werden',
  },
  security: {
    connectedAgentsTitle: 'Verbundene Agents',
    connectedAgentsDescription:
      'KI-Agents, denen du Zugriff auf dein Konto gewährt hast. Trenne alle, die du nicht mehr nutzt — der Zugriff wird sofort entzogen.',
    connectedAgentsDisconnect: 'Trennen',
    connectedAgentsEmpty: 'Keine verbundenen Agents.',
    connectedAgentsAccess: 'Zugriff',
    connectedAgentsGranted: 'Gewährt',
    connectedAgentsLoadFailed: 'Verbundene Agents konnten nicht geladen werden',
    connectedAgentsRevokeFailed: 'Trennen fehlgeschlagen',
    mcpConnectAction: 'Agenten verbinden',
    mcpGuideTitle: 'KI-Agent verbinden',
    mcpGuideDescription:
      'Lass einen KI-Assistenten sicher in deinem Konto arbeiten. Wähle unten deine App und folge den Schritten — es dauert nur eine Minute.',
    mcpServerUrlLabel: 'Serveradresse',
    mcpServerUrlHint: 'Manche Apps fragen einfach nach dieser Adresse.',
    mcpPromptLabel: 'Oder füge dies in deine KI ein',
    mcpPromptHint:
      'Funktioniert mit Chat-Assistenten, die Tools hinzufügen können — füge es in den Chat ein und sende es ab.',
    mcpPromptTemplate:
      'Verbinde dich mit meinem {{name}}-Konto über den MCP-Server unter {{url}} und nutze dann seine Tools, um mir zu helfen.',
    mcpClientsLabel: 'Schritt für Schritt für beliebte Apps',
    mcpAddToConfigFile: 'Öffne die Datei {path} und füge dies hinzu:',
    mcpClientChatgptInstructions:
      'Öffne in ChatGPT Einstellungen → Konnektoren, wähle „Benutzerdefinierten Konnektor hinzufügen“ und füge die obige Serveradresse ein.',
    mcpClientClaudeInstructions:
      'Öffne in Claude Einstellungen → Konnektoren, wähle „Benutzerdefinierten Konnektor hinzufügen“ und füge die obige Serveradresse ein.',
    mcpClientClineInstructions:
      'Öffne in Cline das MCP-Server-Panel, wähle „Remote-Server hinzufügen“ und füge die obige Serveradresse ein.',
    mcpCopy: 'Kopieren',
    mcpCopied: 'Kopiert',
    mcpLearnMore: 'Mehr erfahren',
    inactive: 'Inaktiv',
    inactiveHint:
      'Auf einer früheren Anmeldedomain registriert und nicht mehr verwendbar. Sie können ihn entfernen.',
    passkeysTitle: 'Passkeys',
    passkeysDescription:
      'Mit Passkeys melden Sie sich per Fingerabdruck, Gesicht oder Geräte-PIN an.',
    noPasskeys: 'Keine Passkeys registriert',
    addHint:
      'Um einen Passkey hinzuzufügen, wählen Sie bei der nächsten Anmeldung auf dem gewünschten Gerät "Passkey hinzufügen".',
    rename: 'Umbenennen',
    remove: 'Entfernen',
    removeTitle: 'Passkey entfernen',
    removeDescription: 'Sie können sich mit diesem Passkey nicht mehr anmelden.',
    renamePlaceholder: 'Passkey-Name',
    lastUsed: 'Zuletzt verwendet',
    added: 'Hinzugefügt',
    devicesTitle: 'Ihre Geräte',
    devicesDescription:
      'Telefone und Computer, mit denen Sie sich angemeldet haben. Melden Sie Geräte ab oder entfernen Sie alle, die Sie nicht kennen.',
    devicesEmpty: 'Sie haben sich noch von keinem Gerät aus angemeldet.',
    devicesLoadFailed: 'Ihre Geräte konnten nicht geladen werden. Bitte versuchen Sie es erneut.',
    deviceCurrent: 'Dieses Gerät',
    deviceTrusted: 'Vertrauenswürdig',
    deviceActiveSessions: '{count, plural, one {# aktive Sitzung} other {# aktive Sitzungen}}',
    devicePushEnabled: 'Benachrichtigungen aktiv',
    deviceLastActive: 'Zuletzt verwendet: {date}',
    deviceRename: 'Umbenennen',
    deviceRenamePlaceholder: 'Gerätename',
    deviceRenameFailed: 'Das Gerät konnte nicht umbenannt werden. Bitte versuchen Sie es erneut.',
    deviceSave: 'Speichern',
    deviceCancel: 'Abbrechen',
    deviceSignOut: 'Abmelden',
    deviceSignOutFailed: 'Das Gerät konnte nicht abgemeldet werden. Bitte versuchen Sie es erneut.',
    deviceForget: 'Entfernen',
    deviceForgetFailed: 'Das Gerät konnte nicht entfernt werden. Bitte versuchen Sie es erneut.',
    deviceForgetConfirmTitle: 'Dieses Gerät entfernen?',
    deviceForgetConfirmBody:
      'Dadurch wird das Gerät abgemeldet und aus Ihrer Liste entfernt. Wenn Sie sich erneut damit anmelden, erscheint es wieder hier.',
    sessionsTitle: 'Wo Sie angemeldet sind',
    sessionsDescription:
      'Sie sind derzeit auf diesen Geräten angemeldet. Melden Sie alle ab, die Sie nicht kennen.',
    sessionsEmpty: 'Sie sind nirgendwo sonst angemeldet.',
    sessionsLoadFailed:
      'Ihre Sitzungen konnten nicht geladen werden. Bitte versuchen Sie es erneut.',
    sessionCurrent: 'Diese',
    sessionExpires: 'Abmeldung: {date}',
    sessionLastActive: 'Zuletzt verwendet: {date}',
    sessionSignOut: 'Abmelden',
    sessionSignOutFailed:
      'Diese Sitzung konnte nicht abgemeldet werden. Bitte versuchen Sie es erneut.',
  },
};
