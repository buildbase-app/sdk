...
export default postcss.plugin('postcss-prefix-tailwind', () => {
  return (root) => {
    root.walkRules((rule) => {
      // Process class selectors
      if (rule.selector) {
        // Handle class selectors
        rule.selector = rule.selector.replace(/\.([a-zA-Z0-9_-]+)/g, (match, className) => {
          const prefixedClass = addPrefix(className);
          return `.${prefixedClass}`;
        });
      }
      
      // Process CSS custom properties that might contain class names
      rule.walkDecls((decl) => {
        if (decl.value && decl.value.includes('class')) {
          // This is a bit tricky - we need to be careful not to break CSS
          // For now, we'll focus on the class selectors above
        }
      });
    });
    ...
    root.walkAtRules('apply', (atRule) => {
      if (atRule.params) {
        atRule.params = processClassString(atRule.params);
      }
    });
  };
}); 